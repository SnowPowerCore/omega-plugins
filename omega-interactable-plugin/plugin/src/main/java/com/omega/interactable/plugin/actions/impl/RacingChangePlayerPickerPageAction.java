...
/**
 * Changes the player picker page.
 *
 * additionalInfo keys:
 * - delta: integer (e.g. -1, +1)
 */
public final class RacingChangePlayerPickerPageAction implements InteractableAction {
    ...
    @Inject
    public RacingChangePlayerPickerPageAction(Logger logger, RaceUiApiProvider apiProvider, RacingSessionsApiProvider sessionsProvider) {
        this.logger = logger;
    ...
    public InteractionResult onInteract(InteractionContext context) {
        ...
        try {
            ...
            int next = session.getInt(RacingSessionKeys.PLAYER_PICKER_PAGE, 0) + delta;
            session.set(RacingSessionKeys.PLAYER_PICKER_PAGE, Math.max(0, next));

            api.openEditor(player, RacingUiIds.ADD_RACER, false);
        } catch (Exception e) {
            logger.fine(() -> "RacingChangePlayerPickerPageAction failed: " + e.getMessage());
        }

...
public final class RacingNumberAdjustRaceLapsDelegate implements RacingNumberAdjustSuccessDelegate {
    ...
    @Inject
    public RacingNumberAdjustRaceLapsDelegate(Logger logger, RaceUiApiProvider uiProvider, RaceDataApiProvider dataProvider, RacingSessionsApiProvider sessionsProvider) {
        this.logger = logger;
    ...
    public void adjust(InteractionContext context, int delta) {
        ...
        try {
            ...
            int current = Math.max(1, raceOpt.get().getRace().getLaps());
            int next = clampPositiveIntAdd(current, delta);
            if (data.setRaceLaps(raceName, next)) {
                ui.openEditor(player, (invId == null || invId.isBlank()) ? RacingUiIds.RACE_LAPS : invId, false);
            }
        } catch (Exception e) {
            logger.fine(() -> "RacingNumberAdjustRaceLapsDelegate failed: " + e.getMessage());
        }
    }

...
public final class RacingNumberAdjustQualificationLapsDelegate implements RacingNumberAdjustSuccessDelegate {
    ...
    @Inject
    public RacingNumberAdjustQualificationLapsDelegate(Logger logger, RaceUiApiProvider uiProvider, RaceDataApiProvider dataProvider, RacingSessionsApiProvider sessionsProvider) {
        this.logger = logger;
    ...
    public void adjust(InteractionContext context, int delta) {
        ...
        try {
            ...
            int current = Math.max(1, raceOpt.get().getQualification().getLaps());
            int next = clampPositiveIntAdd(current, delta);
            if (data.setQualificationLaps(raceName, next)) {
                ui.openEditor(player, (invId == null || invId.isBlank()) ? RacingUiIds.QUALIFICATION_LAPS : invId, false);
            }
        } catch (Exception e) {
            logger.fine(() -> "RacingNumberAdjustQualificationLapsDelegate failed: " + e.getMessage());
        }
    }

    private static int clampPositiveIntAdd(int value, int delta) {

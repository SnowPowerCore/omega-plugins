...
public final class RacingNumberAdjustPositionsDelegate implements RacingNumberAdjustSuccessDelegate {
    ...
    @Inject
    public RacingNumberAdjustPositionsDelegate(Logger logger, RaceUiApiProvider uiProvider, RaceDataApiProvider dataProvider, RacingSessionsApiProvider sessionsProvider) {
        this.logger = logger;
    ...
    public void adjust(InteractionContext context, int delta) {
        ...
        try {
            ...
            int current = Math.max(1, raceOpt.get().getPositions());
            int next = clampPositiveIntAdd(current, delta);
            if (data.setPositions(raceName, next)) {
                ui.openEditor(player, (invId == null || invId.isBlank()) ? RacingUiIds.POSITIONS : invId, false);
            }
        } catch (Exception e) {
            logger.fine(() -> "RacingNumberAdjustPositionsDelegate failed: " + e.getMessage());
        }
    }

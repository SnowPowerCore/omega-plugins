...
/**
 * Changes the racers list page.
 *
 * additionalInfo keys:
 * - delta: integer (e.g. -1, +1)
 */
public final class RacingChangeRacerPageAction implements InteractableAction {
    ...
    @Inject
    public RacingChangeRacerPageAction(Logger logger, RaceUiApiProvider apiProvider, RacingSessionsApiProvider sessionsProvider) {
        this.logger = logger;
    ...
    public InteractionResult onInteract(InteractionContext context) {
        ...
        try {
            ...
            int next = session.getInt(RacingSessionKeys.RACERS_PAGE, 0) + delta;
            session.set(RacingSessionKeys.RACERS_PAGE, Math.max(0, next));

            String invId = session.getString(RacingSessionKeys.CURRENT_INVENTORY_ID);
            api.openEditor(player, (invId == null || invId.isBlank()) ? RacingUiIds.RACERS : invId, false);
        } catch (Exception e) {
            logger.fine(() -> "RacingChangeRacerPageAction failed: " + e.getMessage());
        }

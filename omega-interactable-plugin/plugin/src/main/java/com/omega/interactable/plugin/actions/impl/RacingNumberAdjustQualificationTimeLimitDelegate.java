...
public final class RacingNumberAdjustQualificationTimeLimitDelegate implements RacingNumberAdjustSuccessDelegate {
    ...
    @Inject
    public RacingNumberAdjustQualificationTimeLimitDelegate(Logger logger, RaceUiApiProvider uiProvider, RaceDataApiProvider dataProvider, RacingSessionsApiProvider sessionsProvider) {
        this.logger = logger;
    ...
    public void adjust(InteractionContext context, int delta) {
        ...
        try {
            ...
            int current = Math.max(0, raceOpt.get().getQualification().getTimeLimitSeconds());
            int next = clampNonNegativeIntAdd(current, delta);
            if (data.setQualificationTimeLimitSeconds(raceName, next)) {
                ui.openEditor(player, (invId == null || invId.isBlank()) ? RacingUiIds.QUALIFICATION_TIME_LIMIT : invId, false);
            }
        } catch (Exception e) {
            logger.fine(() -> "RacingNumberAdjustQualificationTimeLimitDelegate failed: " + e.getMessage());
        }
    }

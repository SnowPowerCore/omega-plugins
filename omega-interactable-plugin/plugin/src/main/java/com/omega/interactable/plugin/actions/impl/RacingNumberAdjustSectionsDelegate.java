...
public final class RacingNumberAdjustSectionsDelegate implements RacingNumberAdjustSuccessDelegate {
    ...
    @Inject
    public RacingNumberAdjustSectionsDelegate(Logger logger, RaceUiApiProvider uiProvider, RaceDataApiProvider dataProvider, RacingSessionsApiProvider sessionsProvider) {
        this.logger = logger;
    ...
    public void adjust(InteractionContext context, int delta) {
        ...
        try {
            ...
            int current = Math.max(1, raceOpt.get().getSections());
            int next = clampPositiveIntAdd(current, delta);
            if (data.setSections(raceName, next)) {
                ui.openEditor(player, (invId == null || invId.isBlank()) ? RacingUiIds.SECTIONS : invId, false);
            }
        } catch (Exception e) {
            logger.fine(() -> "RacingNumberAdjustSectionsDelegate failed: " + e.getMessage());
        }
    }
